...
eof = Parser \s ->
  case s of
    { str, pos } | pos < S.length str -> Left { pos, error: ParseError { msg: "Expected EOF", suggestions: Nil } }
    _ -> Right { result: unit, suffix: s }

-- | Match any character.
anyChar :: Parser Char
anyChar = Parser \{ str, pos } ->
  case S.charAt pos str of
...
string nt = Parser \s ->
  case s of
    { str, pos } | S.indexOf' (Pattern nt) pos str == Just pos -> Right { result: nt, suffix: { str, pos: pos + S.length nt } }
  ...
  where
    ...
    mkSuggestion pos str = map (\p -> { autoComplete: p, suggestion: nt }) $ S.stripPrefix (Pattern $ S.drop pos str) nt
...
regex pat =
  ...
  where
    ...
    pattern =
      case S.stripPrefix (Pattern "^") pat of
        Nothing ->
          "^" <> pat
        _ ->
          pat
    ...
    matchRegex r =
      Parser \{ str, pos } ->
        let
          remainder = S.drop pos str
        in
          case NEA.head <$> Regex.match r remainder of
            Just (Just matched)  ->
              Right { result: matched, suffix: { str, pos: pos + S.length matched } }
            _ ->
